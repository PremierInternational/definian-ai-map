...
<body>
  ...
  <script type="text/babel">
    ...
    function NodeCard({node,depth,filters,onEdit,onDelete,onAddChild,isRoot}){
      ...
      const depthAccent=depth===0?BRAND.green:depth===1?BRAND.blue:BRAND.darkGray;
      const headerBg=depth===0?BRAND.midnightLight:depth===1?"#071040":"#0d1535";
      function npf(n){if(!n.children||n.children.length===0){return(!filters.status||n.status===filters.status)&&(!filters.owner||n.owner===filters.owner)&&(!filters.buildType||n.buildType===filters.buildType);}return n.children.some(c=>npf(c));}
      ...
      return(<div style={{marginLeft:depth===0?0:20,marginTop:8}}>
        <div style={{border:\`1.5px solid \${depthAccent}\`,borderRadius:10,background:BRAND.midnightLight,boxShadow:"0 2px 16px rgba(0,0,0,0.4)",overflow:"hidden"}}>
          ...
          {adding&&(<div style={{padding:"12px 14px",background:"#030b22",display:"flex",flexWrap:"wrap",gap:10,borderTop:\`1px solid \${BRAND.green}44\`}} onClick={e=>e.stopPropagation()}>
            <div style={{width:"100%",fontSize:11,color:BRAND.green,fontFamily:"'Titillium Web',sans-serif",letterSpacing:"0.08em",textTransform:"uppercase",marginBottom:2}}>＋ Add child to "{node.name}"</div>
            <Field label="Name *"><input style={iS} value={newNode.name} onChange={e=>setNewNode(f=>({...f,name:e.target.value}))} placeholder="Agent name"/></Field>
    ...
    function App(){
      ...
      const[filters,setFilters]=useState({status:"",owner:"",buildType:""});
        ...
        <div style={{maxWidth:860,margin:"0 auto"}}>
          <div style={{display:"flex",alignItems:"center",gap:18,marginBottom:6}}>
            ...
            <div>
              ...
              <div style={{fontSize:12,color:BRAND.coolGray,marginTop:2}}>Data Platform · AI Agents &amp; Assistants Registry</div>
            </div>
          </div>
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:10,alignItems:"center",marginBottom:20}}>
            ...
            <div style={{background:"#0d1a4a",color:BRAND.coolGray,border:"1px solid #0D2C71",borderRadius:8,padding:"5px 14px",fontSize:12,fontWeight:600}}>{leaves.length} Total Agents</div>
            <div style={{flex:1}}/>
...
export default function App() {
  ...
  const counts = useMemo(() => {
    ...
    leaves.forEach(l => { if (c[l.status] !== undefined) c[l.status]++; });
    return c;
  }, [leaves]);
    ...
    <div style={{ fontFamily: "Roboto, sans-serif", background: BRAND.midnight, minHeight: "100vh", padding: 28 }}>
      ...
      <div style={{ maxWidth: 860, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 10, alignItems: "center", marginBottom: 20 }}>
          ...
          <div style={{ background: "#0d1a4a", color: BRAND.coolGray, border: `1px solid ${BRAND.blue}`, borderRadius: 8, padding: "5px 14px", fontSize: 12, fontWeight: 600 }}>
            {leaves.length} Total Agents
          </div>
          <div style={{ flex: 1 }} />
          ...
          {(filters.status || filters.owner || filters.buildType) && (
            <button onClick={() => setFilters({ status: "", owner: "", buildType: "" })}
